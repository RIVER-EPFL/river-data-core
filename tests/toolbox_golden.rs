...
fn golden_chla_benthic_replicates() {
    ...
    for case in &g.modules.chlorophyll["chla_benthic_replicates"] {
        ...
        check_map_value(map, "chla_noacid_ug_l_avg", result.chla_noacid_ug_l_avg, tol, &ctx);
        check_map_value(map, "chla_noacid_ug_l_sd", result.chla_noacid_ug_l_sd, tol, &ctx);
        check_map_value(map, "chla_noacid_ug_m2_avg", result.chla_noacid_ug_m2_avg, tol, &ctx);
        check_map_value(map, "chla_noacid_ug_m2_sd", result.chla_noacid_ug_m2_sd, tol, &ctx);
        if map.contains_key("chla_acid_ug_l_avg") {
            check_map_value(map, "chla_acid_ug_l_avg", opt(result.chla_acid_ug_l_avg), tol, &ctx);
            check_map_value(map, "chla_acid_ug_l_sd", opt(result.chla_acid_ug_l_sd), tol, &ctx);
            check_map_value(map, "chla_acid_ug_m2_avg", opt(result.chla_acid_ug_m2_avg), tol, &ctx);
            check_map_value(map, "chla_acid_ug_m2_sd", opt(result.chla_acid_ug_m2_sd), tol, &ctx);
        } else {
            assert!(result.chla_acid_ug_l_avg.is_none(), "{ctx}: expected no acid results");
        }
        if map.contains_key("afdm_g_m2_avg") {
            ...
            assert!(result.afdm_g_m2_avg.is_none(), "{ctx}: expected no AFDM results");
        }
        count += 1;
    }
...
fn golden_benthic() {
    ...
    for case in &g.modules.benthic["benthic_chla_per_m2"] {
        let dims = v_vec(&case.inputs, "dims_cm");
        check(toolbox::benthic::benthic_chla_per_m2(v(&case.inputs, "chla_ug_l"), &dims, v(&case.inputs, "vol_filtrated"), v(&case.inputs, "vol_total")), case, "benthic", "benthic_chla_per_m2");
        count += 1;
    }
...
fn golden_field_data() {
    ...
    for case in &g.modules.field_data["select_pressure"] {
        ...
        check(result.unwrap_or(f64::NAN), case, "field_data", "select_pressure");
        count += 1;
    }
...
fn golden_pco2() {
    ...
    for case in &g.modules.pco2["pco2_from_co2aq"] {
        ...
        check(toolbox::pco2::pco2_from_co2aq(v(&case.inputs, "co2_aq"), v(&case.inputs, "water_temp_c"), &gc), case, "pco2", "pco2_from_co2aq");
        count += 1;
    }
    for case in &g.modules.pco2["pco2_p1"] {
        ...
        check(toolbox::pco2::pco2_p1(v(&case.inputs, "co2_aq"), v(&case.inputs, "water_temp_c"), v(&case.inputs, "bp_hpa"), &gc), case, "pco2", "pco2_p1");
        count += 1;
    }
    for case in &g.modules.pco2["pco2_p2"] {
        ...
        check(toolbox::pco2::pco2_p2(v(&case.inputs, "co2_aq"), v(&case.inputs, "water_temp_c"), v(&case.inputs, "bp_hpa"), &gc), case, "pco2", "pco2_p2");
        count += 1;
    }
    for case in &g.modules.pco2["dissolved_ch4"] {
        ...
        check(toolbox::pco2::dissolved_ch4(v(&case.inputs, "ch4_dry"), v(&case.inputs, "water_temp_c"), v(&case.inputs, "bp_hpa"), v(&case.inputs, "lab_temp_c"), &gc), case, "pco2", "dissolved_ch4");
        count += 1;
    }
...
fn golden_pco2_replicates() {
    ...
    for case in &g.modules.pco2["pco2_replicates"] {
        ...
        check_map_value(map, "ch4_dissolved_avg", rep.ch4_dissolved_umol_avg, tol, &ctx);
        check_map_value(map, "ch4_dissolved_sd", rep.ch4_dissolved_umol_sd, tol, &ctx);
        count += 1;
    }
...
fn golden_dic_replicates() {
    ...
    for case in &g.modules.dic["dic_replicates"] {
        ...
        let rep = toolbox::dic::dic_replicates(
            v(a, "acid_sample_wght"), v(a, "acid_wght"), v(a, "vol_overpressure"),
            v(a, "sa_added"), v(a, "co2_dry"), v_opt(a, "delta_13co2"),
            v(b, "acid_sample_wght"), v(b, "acid_wght"), v(b, "vol_overpressure"),
            v(b, "sa_added"), v(b, "co2_dry"), v_opt(b, "delta_13co2"),
...
fn golden_nutrients() {
    ...
    for case in &g.modules.nutrients["multi_nutrient_replicates"] {
        ...
        let no3 = results.get("NO3").unwrap_or_else(|| panic!("{ctx}: missing NO3"));
